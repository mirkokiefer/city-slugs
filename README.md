...
One canonical, URL-safe slug for each of 233,819 cities and towns worldwide
(every GeoNames populated place with population ≥ 500), fixed by a
deterministic rule. Born in [Zeit, a world clock for iPhone, Mac, and the
web](https://zeit.xyz) that shows every city's local time under its live sky —
there, these slugs make share URLs readable: `zeit.xyz/?c=heidelberg,paloalto`.

[![Every city in the dataset, drawn as a dot](assets/map.png)](https://mirkokiefer.github.io/city-slugs/)

*The image is the dataset — 233,819 dots, nothing else. Click for the
...
```js
import { cities, bySlug } from "city-slugs";
cities("100k");                    // 6,108 cities ≥ 100k, population-sorted
bySlug("15k").get("paloalto");     // { name: "Palo Alto", country: "US", ... }
```
...
**SQLite** — `data/city-slugs.db`, table `cities`, indexed on `slug`, `name`,
`population`.
...
| file | cities | scope |
|---|---|---|
| `city-slugs.csv` / `.json` / `.parquet` / `.db` | 233,819 | population ≥ 500 |
| `city-slugs-15k.csv` / `.json` | 33,835 | population ≥ 15k |
| `city-slugs-100k.csv` / `.json` | 6,108 | population ≥ 100k |
| `city-slugs-1m.csv` / `.json` | 561 | population ≥ 1M |
| `city-slugs.min.json` | 233,819 | compact `[slug,name,lat,lng,pop]` rows for maps |

Columns: `slug,name,country,region,timezone,lat,lng,population`. Coordinates
